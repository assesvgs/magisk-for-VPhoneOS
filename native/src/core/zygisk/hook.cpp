...
int ZygiskModule::connectCompanion() const {
    if (int fd = zygisk_request(static_cast<int>(ZygiskRequest::ConnectCompanion)); fd >= 0) {
        write_int(fd, id);
        return fd;
    }
    return -1;
...
int ZygiskModule::getModuleDir() const {
    if (int fd = zygisk_request(static_cast<int>(ZygiskRequest::GetModDir)); fd >= 0) {
        write_int(fd, id);
        int dfd = recv_fd(fd);
        close(fd);
        return dfd;

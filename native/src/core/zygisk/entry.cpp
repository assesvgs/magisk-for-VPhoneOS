...
// NOTE: daemon-side handler replaced by Rust daemon.rs::MagiskD::zygisk_handler()
// — this C++ version is kept for reference only and is NOT called.
void zygisk_handler(int client, const sock_cred *cred) {
    int code = read_int(client);
    char buf[256];
    switch (code) {
    case static_cast<int>(ZygiskRequest::GetInfo):
        get_process_info(client, cred);
        break;
    case static_cast<int>(ZygiskRequest::ConnectCompanion):
        if (get_exe(cred->pid, buf, sizeof(buf))) {
        ...
        break;
    case static_cast<int>(ZygiskRequest::GetModDir):
        get_moddir(client);
        break;
    case static_cast<int>(ZygiskRequest::SulistRootNs):
        mount_magisk_to_remote(client, cred);
        break;
    case static_cast<int>(ZygiskRequest::RevertUmount): {
